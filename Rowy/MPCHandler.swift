...
class MPCHandler: NSObject, MCSessionDelegate {
    ...
    func setupBrowser(){
        browser = MCBrowserViewController(serviceType: "rowSession", session: session)
    }
    
    func advertiseSelf(advertise:Bool){
        if advertise{
            advertiser = MCAdvertiserAssistant(serviceType: "rowSession", discoveryInfo: nil, session: session)
            advertiser!.start()
        }else{
            advertiser!.stop()
            advertiser = nil
